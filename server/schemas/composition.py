...
class ImageSlide(BaseModel):
    id: str = Field(default_factory=lambda: f"img_{uuid.uuid4().hex[:8]}")
    image_uri: str
    position_ms: int = 0
...
class Composition(BaseModel):
    ...
    def add_image_slide(self, **kwargs) -> str:
        slide = ImageSlide(**kwargs)
        self.image_slides.append(slide)
        return slide.id
    ...
    def total_duration_ms(self) -> int:
        ...
        for s in self.sequences:
            candidates.append(s.position_ms + int((s.end_ms - s.start_ms) / s.speed))
        for img in self.image_slides:
            candidates.append(img.position_ms + img.duration_ms)
    ...
    def to_timeline_json(self) -> dict:
        ...
        for i, img in enumerate(self.image_slides):
            layers.append({
                "type": "image_slide",
                "z_index": i,
                "source": img.image_uri,
                "start_ms": img.position_ms,
