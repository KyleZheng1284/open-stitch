...
async def render_composition(composition: Composition, project_id: str | None = None) -> str:
    ...
    if is_mock(sandbox_id):
        ...
        result = await _ffmpeg_fallback(composition)
        if project_id:
            emit(project_id, "render.end", node_id="render", outputUri=result)
        return result
        ...
        async with httpx.AsyncClient(timeout=600.0) as client:
            ...
            for layer in timeline["layers"]:
                if "source" in layer and layer["source"]:
                    # Skip sources that are already full URLs (overlays/audio from tools)
                    if not layer["source"].startswith("http"):
            ...
            # Trigger Remotion render (with fallback: strip overlays/audio on failure)
            ...
            if result.get("exit_code", 1) != 0:
                ...
                logger.warning("Render failed, retrying without overlays/audio: %s", error_msg[:200])

                # Fallback: keep video, image, and subtitle layers
                timeline["layers"] = [
                    l for l in timeline["layers"]
                    if l.get("type") in ("video", "image_slide", "subtitle")
                ]
                resp = await client.post(f"{base_url}/remotion/render", json={
                    "timeline": timeline,
                    "output": output_container,
                ...
                if result.get("exit_code", 1) != 0:
                    logger.error("Fallback render also failed: %s", result.get("stderr", ""))
                    raise RuntimeError(f"Render failed: {result.get('stderr', 'unknown error')}")
                logger.info("Fallback render succeeded (without overlays/audio)")
            ...
            if project_id:
                from server.events import emit
                emit(project_id, "render.end", node_id="render", outputUri=host_output)
            return host_output
