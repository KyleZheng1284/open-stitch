...
class EditClip(BaseModel):
    ...
    def _check_range(self) -> "EditClip":
        if self.end_s <= self.start_s:
            raise ValueError("clip end_s must be greater than start_s")
        return self
...
class EditSubtitle(BaseModel):
    ...
    @model_validator(mode="after")
    def _check_range(self) -> "EditSubtitle":
        if self.output_end_s <= self.output_start_s:
            raise ValueError("subtitle output_end_s must be greater than output_start_s")
        return self


class EditTransition(BaseModel):
    type: Literal["crossfade", "cut", "fade_to_black"]
    between_clips: list[int] = Field(default_factory=list)
    duration_ms: int = Field(default=0, ge=0)


class EditMusic(BaseModel):
    mood: str = ""
    energy: str = ""
...
class EditPlan(BaseModel):
    ...
    subtitles: list[EditSubtitle] = Field(default_factory=list)
    transitions: list[EditTransition] = Field(default_factory=list)
    music: EditMusic = Field(default_factory=EditMusic)
    title: str = ""
...
def _extract_length_band(structured_prompt: str) -> tuple[float, float] | None:
    match = re.search(r"-\s*video_length:\s*(.+)", structured_prompt, flags=re.IGNORECASE)
    if not match:
        return None
    chosen = match.group(1).strip().lower()
    if "short" in chosen:
        return (15.0, 60.0)
    if "long" in chosen:
        return (60.0, 300.0)
...
def _apply_guardrails(edit_plan: EditPlan, structured_prompt: str) -> EditPlan:
    max_clips = 24
    max_subtitles = 80

    band = _extract_length_band(structured_prompt)
    if band is not None:
        min_s, max_s = band
        if not (min_s <= edit_plan.total_duration_s <= max_s):
            raise ValueError(
                f"total_duration_s={edit_plan.total_duration_s:.1f} is outside requested range {min_s:.0f}-{max_s:.0f}s"
            )

    trimmed = edit_plan.model_copy(deep=True)
    if len(trimmed.clips) > max_clips:
        logger.warning("Trimming clips from %d to %d", len(trimmed.clips), max_clips)
        trimmed.clips = trimmed.clips[:max_clips]
    if len(trimmed.subtitles) > max_subtitles:
        logger.warning("Trimming subtitles from %d to %d", len(trimmed.subtitles), max_subtitles)
        trimmed.subtitles = trimmed.subtitles[:max_subtitles]

    if not trimmed.clips:
        raise ValueError("edit plan has no clips after guardrails")
    return trimmed
...
async def generate_edit_plan(structured_prompt: str, ingestion_data: dict) -> tuple[EditPlan, dict[str, str]]:
    ...
    try:
        parsed = EditPlan.model_validate(_parse_json(raw))
    except (json.JSONDecodeError, ValidationError, ValueError) as err:
        logger.warning("Edit plan validation failed; retrying once with fix prompt: %s", err)
        fix_messages = [
            {
                "role": "system",
                "content": (
                    "Fix this JSON to match the required schema exactly. "
                    "Return only valid JSON and keep original intent."
                ),
            },
            {
                "role": "user",
        ...
        fixed_raw = await _call_llm(fix_messages, retries=1)
        parsed = EditPlan.model_validate(_parse_json(fixed_raw))

    guarded = _apply_guardrails(parsed, structured_prompt)
    return guarded, key_to_path
...
def build_composition(edit_plan: dict, key_to_path: dict[str, str], ingestion_data: dict) -> Composition:
    ...
    for clip in edit_plan.get("clips", []):
        ...
        if not video_path:
            raise ValueError(
                f"Unknown source_video '{source_key}'. Expected one of: {sorted(k for k in key_to_path if k.startswith('video_'))}"
            )
...
async def run_editing_agent(project: dict) -> str:
    ...
    logger.info("Edit plan:\n%s", json.dumps(edit_plan.model_dump(), indent=2)[:2000])

    # Step 2: Convert to Remotion Composition
    logger.info("Building composition...")
    composition = build_composition(edit_plan.model_dump(), key_to_path, ingestion_data)
