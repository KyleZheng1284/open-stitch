...
import { CloudUpload, Upload as UploadIcon, X, Send } from "lucide-react";
...
function UploadContent() {
  ...
  return (
    <div className="h-screen bg-[#121212] flex overflow-hidden">
            ...
            <div className={`flex items-center justify-center gap-8 ${uploadedFiles.length === 0 ? "flex-col" : ""}`}>
              ...
              <div className={uploadedFiles.length === 0 ? "text-center" : "text-left"}>
                <h3 className={`font-bold text-white mb-2 ${uploadedFiles.length === 0 ? "text-3xl" : "text-xl"}`}>
                  {uploadedFiles.length > 0 ? "Add More Files" : "Drag & Drop Videos or Photos Here"}
                </h3>
                <p className={`text-gray-400 ${uploadedFiles.length === 0 ? "text-lg mb-6" : "text-sm mb-3"}`}>
                  Videos (MP4, MOV) and photos (JPG, PNG, WebP)
                </p>
      ...
      {uploadedFiles.length > 0 && (
        <motion.div
          className="w-96 bg-[#1a1a1a] border-l border-gray-800 flex flex-col h-full shrink-0"
