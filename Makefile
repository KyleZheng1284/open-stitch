...
#   make test-ci     — Run lint + type + tests for graph + legacy flow checks
...
dev: ## Start backend (uvicorn) + frontend (next dev) in parallel
	...
	@trap 'kill 0' EXIT; \
		$(PYTHON_BIN) -m uvicorn server.main:app --port 8080 --reload & \
...
.PHONY: test-ci
test-ci: ## Run CI-style checks (ruff + mypy + pytest)
	./tools/test_all.sh

.PHONY: lint
lint: ## Run ruff linter for graph and tests
	python3 -m ruff check server/graph server/agents/tools.py tests

.PHONY: format
format: ## Auto-format code
	python3 -m ruff check --fix server/graph server/agents/tools.py tests
	python3 -m ruff format server/graph server/agents/tools.py tests

.PHONY: typecheck
typecheck: ## Run focused mypy checks for graph runtime
